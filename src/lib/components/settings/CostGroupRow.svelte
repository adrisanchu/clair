<script lang="ts">
	...
	interface Props {
		costGroup: CostGroupRow;
		isOwner: boolean;
	}

	let { costGroup, isOwner }: Props = $props();
	...
	function startRename() {
		isRenaming = true;
		renameValue = costGroup.name;
	}

	async function submitRename() {
		const trimmed = renameValue.trim();
		isRenaming = false;
		if (!trimmed || trimmed === costGroup.name) return;

		await fetch(`/api/settings/cost-groups/${costGroup.id}`, {
			method: 'PATCH',
	...
	async function confirmDelete() {
		deleteOpen = false;
		isDeleting = true;
		const res = await fetch(`/api/settings/cost-groups/${costGroup.id}`, { method: 'DELETE' });
		if (res.ok) {
			await invalidateAll();
		}
		isDeleting = false;
...
<div
	class="flex items-center gap-3 rounded-lg px-3 py-2.5 transition-colors hover:bg-surface-sunken {isDeleting
		? 'opacity-50'
		: ''}"
>
	<!-- Color dot -->
	<span class="h-3 w-3 shrink-0 rounded-full" style="background-color: {costGroup.color}"></span>
	...
	<div class="min-w-0 flex-1">
		{#if isRenaming}
			...
			/>
		{:else}
			<span class="truncate text-sm font-medium text-text-primary">{costGroup.name}</span>
		{/if}
	</div>
...
<CostGroupFormSheet bind:open={editOpen} mode="edit" {costGroup} />

<AlertDialog.Root bind:open={deleteOpen}>
	<AlertDialog.Content>
		<AlertDialog.Header>
			<AlertDialog.Title>Delete "{costGroup.name}"?</AlertDialog.Title>
			<AlertDialog.Description>
				Transactions in this cost group will keep the label. This action cannot be undone.
			</AlertDialog.Description>
		</AlertDialog.Header>
		<AlertDialog.Footer>
